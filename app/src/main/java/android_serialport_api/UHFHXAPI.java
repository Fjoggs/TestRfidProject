...
public class UHFHXAPI {
    ...
    private class Packet {
        ...
        public Packet(byte messageType, byte messageCode, byte[] payloadData) {
            ...
            if (payloadData != null) {
                Log.i("whw",
                        "payloadData=" + DataUtils.toHexString(payloadData));
            }
        }
    ...
    public int transceive(byte messageType, byte messageCode, byte[] data) {
        ...
        int len = SerialPortManager.getInstance().readFixedLength(buffer, 3000,
                0);
        newbuffer = new byte[len - 14];
        System.arraycopy(buffer, 14, newbuffer, 0, newbuffer.length);
        if (len > 0) {
    ...
    /**
     * Function Description: Get 18000-6C air interface protocol command \A1\AEQuery\A1\AF
     * parameters.
     *
    ...
    /**
     * Function Description: Set 18000-6C air interface protocol command \A1\AEQuery\A1\AF
     * parameters.
     *
     * @param arguments
    ...
    public void startAutoRead(int commandCode, byte[] rc,
        ...
        int mm = SerialPortManager.getInstance().read(buffer, 300, 30);
    ...
    public Response readTypeCTagData(byte[] arguments) {
        ...
        if (length == 0) {
            ...
            if (newbuffer[2] != MessageCode.Read_Type_C_Tag_Data) {

                response.result = Response.COMMAND_FAILURE;
            } else {
                short payloadLength = DataUtils.getShort(newbuffer[3],
                        newbuffer[4]);
                byte[] data = new byte[payloadLength];
                System.arraycopy(newbuffer, 5, data, 0, data.length);
                response.result = Response.RESPONSE_PACKET;
                response.data = data;
            }
        }
    ...
    public Response writeTypeCTagData(byte[] arguments) {
        ...
        if (length == 0) {
            ...
            if (newbuffer[2] != MessageCode.Write_Type_C_Tag_Data) {
                response.result = Response.COMMAND_FAILURE;
            } else {
                short payloadLength = DataUtils.getShort(newbuffer[3],
                        newbuffer[4]);
     ...
     *            UL (16-bit): Target tag\A1\AFs EPC length - EPC (variable): Target
     *            tag\A1\AFs EPC - MB (8-bit): Target memory bank; 0x00 Reserved,
     ...
     *            password protected. Otherwise, set AP filed to 0x00000000. -
     *            UL (16-bit): Target tag\A1\AFs EPC length - EPC (variable): Target
     *            tag\A1\AFs EPC - MB (8-bit): Target memory bank; 0x00 RFU, 0x01
     ...
     *            password protected. Otherwise, set AP filed to 0x00000000. -
     *            UL (16-bit): Target tag\A1\AFs EPC length - EPC (variable): Target
     *            tag\A1\AFs EPC - RFU (8-bit): 0x00 - R/L (8-bit): Read/Lock bit;
     ...
     *            \A1\AEKill Type C Tag\A1\AF command do not work. The target tag ignores
     *            it. - UL (16-bit): Target tag\A1\AFs EPC length - EPC (variable):
     *            Target tag\A1\AFs EPC - Recom (8-bit): Recommissioning bits
     ...
     *            (16-bit): Target tag\A1\AFs EPC length - EPC (variable): Target
     *            tag\A1\AFs EPC - LD (24-bit): Lock mask and action flags. Pad 4-bit
     ...
     *
     * @return - Temp (8-bit): Current temperature Example) 24 \A1\E3C
     */
     ...
     *         CHS + 1 (-dBm) \A1\AD. - RSSI[N] (8-bit): RSSI level on CHE (-dBm)
    ...
    private byte[] topackage(byte[] old) {
        int len = old.length;
        byte[] heade = { (byte) 0xCA, (byte) 0xDF, (byte) 0x01, (byte) 0x35 };
