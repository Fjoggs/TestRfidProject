...
public class UHFHXAPI {
	...
	private static final class MessageCode {
		...
		public static final byte Get_Tx_Power_Level = (byte) 0xD4;
	...
	public boolean open() {
		...
		int len = SerialPortManager.getInstance().readFixedLength(buffer, 3000,
				requestLength);
		Log.i("whw", "tttt="+new String(buffer));
	...
	public int transceive(byte messageType, byte messageCode, byte[] data) {
		...
		if(len > 0){
			...
			Log.i("whw", "temp="+DataUtils.toHexString(temp));
		}
		return len;
	}
	/**
	 * Function Description: Set power mode
	 * @return
	 */
	public void setReaderPowerMode(byte mode) {
		int length = transceive(MessageType.Command,
				MessageCode.Set_Reader_Power_Control, new byte[] { mode });
	}
	...
	/**
	 * Function Description: Get the current region. PR9200 uses individual channel table that depends
	 * on region. List of region code follows below.
	 * 
	...
	/**
	 * Function Description: Set the current region. PR9200 uses individual channel table that depends
	 * on region. List of region code follows below. - Korea (0x11)<br>
	 * - US (0x21)<br>
	 * - US2 (0x22)<br>
	 * - Europe (0x31)<br>
	 * - Japan (0x41)<br>
	...
	/**
	 * Function Description: Set 18000-6C air interface protocol command ��Query�� parameters.
	 * 
	 * @param arguments
	...
	public Response getTxPowerLevel() {
		int length = transceive(MessageType.Command,
				MessageCode.Get_Tx_Power_Level, new byte[]{0x00,0x03});
		Response response = new Response();
		if (length == 0) {
			response.result = Response.TIME_OUT;
		} else {
	...
	/**
	 * Function Description: Turn the Continuous Wave (CW) signal on/off. This command packet is only
	 * valid for idle mode.
	 * 
	 * @param argument
	 *            - On (0xFF)<br>
	...
	/**
	 * Function Description: Start an automatic tag read operation during the inventory round,
	 * tag IDs are send back to user through notification packet.
	 * 
	 * @return void
	 */
...
//					Log.i("whw", "dataPacket="+dataPacket);
					if(dataPacket != null){
	...
	/**
	 * Function Description: Read Type C tag data from specified memory bank.
	 * 
	 * @param arguments
	 *            - AP (32-bit): Access Password if target memory bank was
	 *            password protected. Otherwise, set AP filed to 0x00000000.<br>
	 *            - UL (16-bit): Target tag��s EPC length<br>
	 *            - EPC (variable): Target tag��s EPC<br>
	 *            - MB (8-bit): Target memory bank; RFU (0x00), EPC (0x01), TID
	 *            (0x02), User (0x03)<br>
	 *            - SA (16-bit): Starting Address word pointer<br>
	 *            - DL (16-bit): Data Length (Word Count)<br>
	 *            Example)<br>
	 *            Access Password = 0x00000000, UL = 12 (0x0C) byte,<br>
	 *            EPC = 0xE2003411B802011526370494, Target memory bank = RFU,
	 *            Start Address = 0x0000, Length = 4 word<br>
	 * @return - Tag memory contents (variable)<br>
	 *         Example) RFU memory bank = 0x0000000000000000
	 */
	...
	/**
	 * Function Description: Get current modulation mode. The modulation mode is combination Rx
	...
	/**
	 * Function Description: Set current modulation mode. The modulation mode is combination Rx
	 * modulation type and BLF
	 * 
	 * @param arguments
	...
	/**
	 * Function Description: Start an automatic tag read operation,
	 * tag IDs are send back to user through notification packet.
	 * 
	 * @return Response
	 */
	...
	/**
	 * Function Description: Write type C tag data.
	 * 
	 * @param arguments
	 *            - AP (32-bit): Access Password if target memory bank was
	...
	/**
	 * Function Description: Sets Registry Update function. - Arg (8-bit): Store (0x01) Example) Store
	 * data into Registry
	 * 
	 * @return Success (0x00).
	 */
...
//			Log.i("whw", "getFullPacket="+DataUtils.toHexString(temp));
			if(packetStartIndex < dataLength){
				if (mBuffer[packetStartIndex] == (byte)0xBB
						&& (mBuffer[packetStartIndex + 1] == UHFHXAPI.MessageType.Response || mBuffer[packetStartIndex + 1] == UHFHXAPI.MessageType.Notification)) {
...
//					Log.i("whw", "**************=");
					if (payloadLength >= 0
						...
						System.arraycopy(mBuffer, tempBuf.length, lastBuffer, 0,
								lastBuffer.length);
//						Log.i("whw", "lastBuffer="+DataUtils.toHexString(lastBuffer));
...
//				Log.i("whw", "@@@@@@@@@@@@@@@@@packetStartIndex="+packetStartIndex+"  dataLength="+dataLength);
	...
	private static class PacketType{
		...
		public static int getPacketType(byte[] data){
			...
			}else if(data[1]== MessageType.Notification && data[2] == MessageCode.Start_Auto_Read && data[5] == 0x1F){
				Log.i("whw", "Auto_Read_End");
